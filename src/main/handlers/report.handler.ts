...
export function registerReportHandlers(db: DB): void {
  ...
  // Yearly report — new channel, add to ipcChannels.ts if not present
  safeHandle('report:yearly', (_e, year: number) => {
    const { buildYearlyReport } = require('../services/reportService')
    return buildYearlyReport(db, year)
  })
}
