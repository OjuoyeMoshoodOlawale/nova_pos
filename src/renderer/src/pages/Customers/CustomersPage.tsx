...
import { Plus, Edit2, Archive, X, History } from 'lucide-react'
...
export default function CustomersPage(){
  ...
  async function save(){
    ...
    setSaving(true)
    const r=editing?await window.api.customers.update(editing.id,d):await window.api.customers.create(d)
    setSaving(false)
  ...
  async function viewHistory(c:Customer){
    const r=await window.api.customers.history(c.id);if(r.success){setHistory(r.data);setHistName(c.full_name)}
  }
  ...
  return(
    <div className="p-6 space-y-5">
      <div className="flex items-center justify-between">
        <div><h1 className="text-2xl font-bold">Customers</h1><p className="text-sm text-slate-500">{customers.length} records</p></div>
        <button onClick={()=>openForm()} className="btn-primary flex items-center gap-2"><Plus className="w-4 h-4"/> Add Customer</button>
      </div>
      <DataTable columns={cols} data={customers} isLoading={loading} searchKeys={['full_name','phone','email']} searchPlaceholder="Search customers..." emptyText="No customers."
        actions={c=><div className="flex gap-1">
