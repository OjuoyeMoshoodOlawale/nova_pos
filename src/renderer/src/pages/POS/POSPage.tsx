...
import { ShoppingCart, X, Plus, Minus, Trash2, Tag, Pause, Zap, Package, Grid } from 'lucide-react'
    ...
    <div className={`bg-white rounded-xl border overflow-hidden transition-all ${out?'opacity-40 border-slate-100':'border-slate-100 hover:border-blue-300 hover:shadow-md'}`}>
      <button onClick={()=>!out&&onAdd(product,'unit')} disabled={out} className="w-full p-3 text-left">
        <div className="flex items-center justify-center h-12 mb-2 rounded-lg overflow-hidden bg-slate-50">
          {product.image_data ? <img src={product.image_data} className="w-full h-full object-cover"/> : <Package className="w-7 h-7 text-slate-200"/>}
        </div>
        <p className="text-xs font-semibold text-slate-800 line-clamp-2 min-h-[2rem]">{product.name}</p>
        <p className="text-sm font-bold text-blue-600 mt-1">{currencySymbol}{product.selling_price.toFixed(2)}<span className="text-xs font-normal text-slate-400 ml-0.5">/{product.unit}</span></p>
    ...
    <div className={`group py-2.5 border-b border-slate-50 last:border-0 transition-colors ${flash?'bg-green-50':''}`}>
      <div className="flex items-start gap-2">
        <div className="flex-1 min-w-0">
          ...
          <p className="text-xs text-slate-400">{currencySymbol}{item.unit_price.toFixed(2)}/{item.unit_label}
        ...
        <div className="flex items-center gap-1 flex-shrink-0">
          <button onClick={()=>updateQty(item.product_id, mode, item.quantity-1)}
            className="w-6 h-6 rounded-full bg-slate-100 hover:bg-red-100 flex items-center justify-center"><Minus className="w-3 h-3"/></button>
        ...
        <button onClick={()=>removeItem(item.product_id, mode)} className="opacity-0 group-hover:opacity-100 text-red-400 hover:text-red-600"><X className="w-4 h-4"/></button>
      </div>
      {atLimit && (
        <p className="text-xs text-amber-600 mt-0.5 flex items-center gap-1">
          ⚠️ Max stock reached ({item.stock_qty} {item.unit_label} available)
        </p>
      )}
      {showDisc ? (
...
export default function POSPage() {
  ...
  const filtered = products.filter(p => (activeCat===null||p.category_id===activeCat) && p.is_active)

  return (
    <div className="flex flex-col h-full overflow-hidden bg-slate-50">
      <CheckoutBar onProductAdded={(product, mode) => handleAddProduct(product, mode as SellMode)}/>

      <div className="flex flex-1 overflow-hidden">
        {/* Left: categories + optional grid */}
          ...
          {gridOpen && (
            <div className="flex-1 overflow-y-auto p-3 border-r border-slate-200">
              {loading
                ...
                : <div className="grid grid-cols-3 gap-2">{filtered.map(p=><ProductCard key={p.id} product={p} onAdd={handleAddProduct}/>)}</div>}
            </div>
          )}
          {!gridOpen && (
        ...
        <div className="flex flex-col bg-white flex-1">
          <div className="px-4 py-3 border-b border-slate-100 flex items-center justify-between">
            ...
            <div className="flex items-center gap-1">
              <button onClick={()=>setShowHold(true)} className="p-1.5 text-slate-400 hover:text-amber-500 rounded-lg"><Pause className="w-4 h-4"/></button>
              {cart.items.length > 0 && <button onClick={()=>cart.clearCart()} className="p-1.5 text-slate-400 hover:text-red-500 rounded-lg"><Trash2 className="w-4 h-4"/></button>}
            </div>
          </div>
          ...
          <div className="px-4 py-2 border-b border-slate-50">
            <CustomerSearch
              ...
              groupName={cart.customerName ? undefined : undefined}
              onSelect={(id,name,disc,grp)=>cart.setCustomer(id,name,disc)}
            />
          </div>
          ...
          <div className="flex-1 overflow-y-auto px-4 py-2">
            {cart.items.length === 0 ? (
              ...
              cart.items.map(item => (
                <CartRow key={cart.getItemKey(item.product_id, item.sell_mode as SellMode)}
                  item={item}
          ...
          {cart.items.length > 0 && (
            <div className="border-t border-slate-100 px-4 py-3 space-y-1.5">
              <div className="flex justify-between text-sm text-slate-500">
                <span>Subtotal</span><span>{cart.currencySymbol}{totals.subtotal.toFixed(2)}</span>
              </div>
              ...
              <div className="flex justify-between text-sm text-slate-500">
                <span>{cart.taxName}</span><span>{cart.currencySymbol}{totals.taxAmount.toFixed(2)}</span>
              </div>
      ...
      {showHold && <HoldOrderModal onClose={()=>setShowHold(false)} onRetrieve={json=>{
        try {
          ...
          r.items?.forEach((i:any)=>{
            const p={...i,id:i.product_id,selling_price:i.unit_price,stock_qty:i.stock_qty??999,reorder_level:0,is_active:true,cost_price:i.cost_price??0,has_bulk_pricing:false,bulk_unit:null,units_per_bulk:1,bulk_buying_price:0,bulk_selling_price:0,image_data:null} as Product
